...
export const FormPreview: React.FC<Props> = ({ onBack }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const missingRequired = components.find(
      c => c.required && c.type !== 'button' && (!formData[c.id] || formData[c.id].length === 0)
    );
  ...
  if (isSubmitted) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center bg-gray-50 h-full">
        ...
        <p className="text-gray-500 mt-2">请按 F12 打开控制台查看收集到的 JSON 数据。</p>
        ...
        >
          ...
          <div className="flex flex-col gap-6">
            {components.map((comp, index) => {
              ...
              if (comp.type === 'button') {
                return (
                  <button key={comp.id} type="submit" className="w-full bg-brand text-white py-3 rounded-md font-bold mt-4 shadow-md hover:bg-brand/90 transition-colors">
            ...
            {!components.some(c => c.type === 'button') && components.length > 0 && (
              <button type="submit" className="w-full bg-brand text-white py-3 rounded-md font-bold mt-4 shadow-md hover:bg-brand/90 transition-colors">
                提交数据
              </button>
            )}
          </div>
