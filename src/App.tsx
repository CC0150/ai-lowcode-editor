...
export default function App() {
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    console.log("拖拽结束", { 拖拽的组件: active.id, 放下的目标: over?.id });
  ...
  return (
    <div className="h-screen w-full flex flex-col bg-gray-50 overflow-hidden font-sans">
      ...
      <main className="flex-1 flex overflow-hidden">
        ...
        >
          <div
            className="w-[1000px] min-h-[570px] bg-white shadow-xl rounded-md relative overflow-hidden transition-all duration-300 ring-1 ring-gray-200"
              ...
              >
                {components.map((comp) => (
                  ...
                  >
                    ...
                    {comp.type === "image" && (
                      <div className="bg-gray-200 h-32 flex items-center justify-center text-gray-500 text-sm w-full">
                        图片占位
                      </div>
                    )}
                    {comp.type === "container" && (
                      <div className="min-h-[100px] border border-dashed border-gray-300 p-2 w-full">
                        容器占位
                      </div>
                    )}
                  </SortableWrapper>
